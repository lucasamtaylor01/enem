...
def carregar_ou_tratar_dados(ano: int):
    ...
    print(f"Dados para modelo de clustering salvos com sucesso em {caminho_modelo_municipio}\n")
    ...
    print(f"Dados para modelo de clustering salvos com sucesso em {caminho_modelo_uf}\n")
...
def tratamento_participantes(df_participantes_raw, ano):
    if ano == 2024:
        df_participantes_raw = df_participantes_raw.rename(columns={'Q007': 'Q006'})
    
    df_participantes = df_participantes_raw[['IN_TREINEIRO', 'SG_UF_PROVA', 'Q006', 'NO_MUNICIPIO_PROVA']]
    ...
    df_municipio = df_participantes.groupby('MUNICIPIO').agg(RENDA_FAMILIAR_SM_MEDIA=('RENDA_FAMILIAR_SM', 'mean')).reset_index()
...
def tratamento_clustering(df_municipio, df_resultado):
    ...
    x_scaled_municipio = escalar_features_clustering(
        df_pre_clustering_municipio,
        ['MUNICIPIO', 'RENDA_FAMILIAR_SM_MEDIA', 'UF', 'QTD_PARTICIPANTES', 'MEDIA_GERAL'],
    )
...
def escalar_features_clustering(df_pre_clustering, colunas_excluir):
    ...
    x_scaled[COLUNAS_NOTAS_CLUSTERING] = scaler.fit_transform(x_scaled[COLUNAS_NOTAS_CLUSTERING])
    return x_scaled
...
def tratamento_de_dados(df_participantes_raw, df_resultado_raw, ano):
    ...
    df_pre_clustering_municipio, x_scaled_municipio = tratamento_clustering(df_participantes, df_resultado)
    ...
    return df_pre_clustering_municipio, x_scaled_municipio, df_pre_clustering_uf, x_scaled_uf
